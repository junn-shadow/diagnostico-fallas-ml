...
def generate_alerts(logs, nlp_backend: str = "auto"):
    ...
    mask = df["is_anomaly"] | df["level"].isin(["WARNING", "WARN", "ERROR", "CRITICAL", "FATAL"])
    ...
    if mask.any():
        ...
        logger.info("Diagnosticando semánticamente %d alertas/plantillas únicas.", len(unique_alerts))
        
        # Ejecutar la inferencia semántica sobre el subconjunto único reducido
        force_backend = _resolve_alert_backend(nlp_backend)
        root_causes, recommendations = infer_semantic_diagnostics(unique_alerts, force_backend=force_backend)
        ...
        df.loc[mask, "root_cause"] = mapped_results.apply(lambda x: x[0] if isinstance(x, tuple) else "Comportamiento normal")
        df.loc[mask, "recommendation"] = mapped_results.apply(lambda x: x[1] if isinstance(x, tuple) else "No requiere accion")
