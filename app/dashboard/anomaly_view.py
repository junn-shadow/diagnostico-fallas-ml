...
def render_anomalies(logs: pd.DataFrame) -> None:
    ...
    with col_badge:
        badge_color = "#ef4444" if total_alerts > 5 else "#f59e0b" if total_alerts > 0 else "#10b981"
        badge_text = "CRÍTICO" if total_alerts > 5 else "ATENCIÓN" if total_alerts > 0 else "ESTABLE"
        st.markdown(
            f"""
            <div style="
                text-align:center;
        ...
        if not sev_counts.empty:
            mini_fig = go.Figure(
                go.Bar(
                    ...
                    marker_color=[COLOR_MAP.get(l, "#64748b") for l in sev_counts.index],
            ...
            mini_fig.update_layout(
                ...
                font=dict(family="Plus Jakarta Sans, sans-serif", size=10, color="#475569"),
    ...
    # Tabla de alertas
    columns = ["line_id", "level", "is_anomaly", "anomaly_score", "event_template", "root_cause", "recommendation"]
    alerts = logs[logs["is_anomaly"] | logs["level"].isin(["ERROR", "CRITICAL", "FATAL"])]

    if alerts.empty:
        st.success("✅ No hay alertas activas con los filtros actuales.")
        return

    min_score = float(logs["anomaly_score"].min()) if not logs["anomaly_score"].isna().all() else 0.0
    max_score = float(logs["anomaly_score"].max()) if not logs["anomaly_score"].isna().all() else 1.0
    if min_score >= max_score:
        max_score = min_score + 1.0

    st.dataframe(
        alerts[columns].sort_values(["is_anomaly", "anomaly_score"], ascending=[False, False]),
        ...
        column_config={
            ...
            "anomaly_score": st.column_config.ProgressColumn(
                "Riesgo ML", min_value=min_score, max_value=max_score,
            ),
