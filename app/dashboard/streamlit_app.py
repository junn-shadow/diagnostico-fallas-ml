...
from app.dashboard.charts import render_charts, render_tradingview_timeline, render_pattern_analysis
...
from app.database.repositories import list_incidents, list_runs, delete_run, search_all_incidents, get_incident_summary
from app.config.settings import LARGE_LOG_THRESHOLD_ROWS, MAX_UPLOAD_SIZE_MB
from app.nlp.model_manager import check_distilbert_status
from app.services.pipeline_service import PipelineService
from app.dashboard.report_generator import generate_html_report, generate_json_report, generate_csv_report
...
_badge_color = "#ef4444" if _hero_anomalies > 5 else "#f59e0b" if _hero_anomalies > 0 else "#10b981"
_badge_label = "Crítico" if _hero_anomalies > 5 else "Atención" if _hero_anomalies > 0 else "Estable"
...
<div>
<span class="eyebrow">DiagnosticOps ML — Plataforma Inteligente de Análisis de Logs</span>
<h1>Centro de Diagnóstico de Fallas Operativas</h1>
<p>Detección no supervisada · Agrupación semántica NLP · Recomendaciones automatizadas para equipos SRE &amp; DevOps</p>
</div>
{hero_badge_html}
</div>
</div>
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("🤖 Modelo NLP Semántico")
    
    nlp_backend = st.selectbox(
        "Backend NLP",
        options=["auto", "tfidf", "distilbert"],
        format_func=lambda x: {
            "auto": "Auto (DistilBERT remoto si hay token, si no TF-IDF)",
            "tfidf": "TF-IDF (Rápido y offline)",
            "distilbert": "DistilBERT Remoto (Hugging Face API)"
        }[x],
        index=0,
    ...
    if model_status.available_remote:
        st.success("✅ DistilBERT conectado via Hugging Face API")
    elif model_status.available_locally:
        st.success("✅ DistilBERT disponible localmente")
    else:
        st.warning("⚠️ Sin token HF_API_TOKEN — Usando TF-IDF como fallback")
        st.caption("Define la variable de entorno HF_API_TOKEN para activar DistilBERT remoto.")

    st.caption(f"Upload max configurado: {MAX_UPLOAD_SIZE_MB} MB")
    run_button = st.button("Ejecutar diagnostico", type="primary", width="stretch")
...
current_params = {
    "source": uploaded.name if uploaded is not None else "sample.log" if use_sample else None,
    "contamination": contamination,
    "clustering_method": clustering_method,
    "clustering_eps": clustering_eps,
    "clustering_min_samples": clustering_min_samples,
...
should_run = run_button or ("last_result" not in st.session_state and auto_run)
...
if "last_result" in st.session_state:
    ...
    with st.sidebar:
        st.divider()
        st.subheader("🔍 Filtrado de Log")
        search_query = st.text_input("Buscador (Palabras clave)", value="", help="Filtra reactivamente los logs por cualquier término.")
        ...
        selected_clusters = st.multiselect("Cluster semantico", clusters, default=clusters)
        only_alerts = st.toggle("Solo alertas", value=False)
        max_rows = st.slider("Filas visibles", 200, 5000, 1000, step=200)

    # Filtrar por búsqueda
    if search_query:
        mask = logs["raw_log"].astype(str).str.contains(search_query, case=False, na=False)
        if "clean_log" in logs.columns:
            mask = mask | logs["clean_log"].astype(str).str.contains(search_query, case=False, na=False)
        logs = logs[mask]
    ...
    if only_alerts:
        logs = logs[logs["is_anomaly"] | logs["level"].isin(["ERROR", "CRITICAL", "FATAL"])]
    ...
    with tab_overview:
        st.markdown('<p class="section-note">Lectura rapida del comportamiento operativo detectado.</p>', unsafe_allow_html=True)
        ...
        semantic_label = "DistilBERT Remoto" if "distilbert" in result.semantic_backend else "TF-IDF fallback"
        model_status = get_distilbert_status()
        badge_class = "ready" if (model_status.available_remote or model_status.available_locally) else "fallback"
        badge_text = "REMOTO" if model_status.available_remote else ("LOCAL" if model_status.available_locally else "FALLBACK")
        st.markdown(
            f"""
        ...
        with st.expander("📥 Descargar Reporte de Diagnóstico", expanded=False):
            ...
            # Generar los payloads de descarga
            ...
            with dl_col4:
                if xlsx_available and len(xlsx_report) > 200: # standard CSV fallback is around 100-200 bytes or more
                    st.download_button(
    ...
    with tab_data:
        ...
        if _total_shown < _total_all:
            st.info(f"Mostrando **{_total_shown:,}** de **{_total_all:,}** eventos. Ajusta 'Filas visibles' en el panel lateral para ver más.")
        st.dataframe(
            visible_logs,
            use_container_width=True,
            hide_index=True,
            column_config={
                ...
                "event_template": st.column_config.TextColumn("Plantilla Drain", width="medium"),
                "anomaly_score": st.column_config.NumberColumn("Score ML", format="%.4f", width="small"),
                "root_cause": st.column_config.TextColumn("Causa Probable", width="medium"),
                "recommendation": st.column_config.TextColumn("Acción Sugerida", width="large"),
            },
        )
    ...
    with tab_history:
        ...
        try:
            ...
            if runs_df.empty:
                ...
                compare_mode = False
                if len(runs_df) >= 2:
                    compare_mode = st.checkbox("🔍 Activar Comparador de Ejecuciones (Comparar dos logs históricos)", value=False)
                
                if compare_mode:
                    col_c1, col_c2 = st.columns(2)
                    with col_c1:
                        sel_a = st.selectbox("Ejecución A (Línea Base)", options=runs_df["display_name"].tolist(), index=1 if len(runs_df) > 1 else 0)
                    with col_c2:
                        sel_b = st.selectbox("Ejecución B (Comparar)", options=runs_df["display_name"].tolist(), index=0)
                    ...
                    if run_id_a == run_id_b:
                        st.warning("⚠️ Selecciona dos ejecuciones distintas para poder compararlas.")
                    else:
                        def metric_delta(val_a, val_b):
                            ...
                            if val_a == 0:
                                ...
                                pct = (diff / val_a) * 100
                            sign = "+" if diff > 0 else ""
                            return f"{sign}{diff} ({sign}{pct:.1f}%)" if diff != 0 else "Sin cambio"
                        ...
                        m_col1.metric(
                            ...
                            delta=metric_delta(row_a['total_events'], row_b['total_events'])
                        )
                        m_col2.metric(
                            label="Anomalías ML", 
                        ...
                        templates_a = set(inc_a["event_template"].dropna().unique().tolist())
                        templates_b = set(inc_b["event_template"].dropna().unique().tolist())
                        ...
                        exp_new = st.expander(f"🆕 Nuevos Patrones en Ejecución B ({len(new_templates)})", expanded=True)
                        with exp_new:
                            if new_templates:
                                st.markdown("Los siguientes patrones de logs ocurrieron en la Ejecución B pero **no estaban presentes** en la Ejecución A:")
                                for t in list(new_templates)[:20]:
                                    st.code(t, language="text")
                                if len(new_templates) > 20:
                                    st.caption(f"... y {len(new_templates) - 20} patrones más.")
                            else:
                                st.success("✅ No se encontraron patrones de error nuevos en la Ejecución B.")
                                
                        exp_res = st.expander(f"✅ Patrones Resueltos/Ausentes en Ejecución B ({len(resolved_templates)})", expanded=False)
                        with exp_res:
                            if resolved_templates:
                                st.markdown("Los siguientes patrones de logs ocurrieron en la Ejecución A pero **ya no están presentes** en la Ejecución B:")
                                for t in list(resolved_templates)[:20]:
                                    st.code(t, language="text")
                                if len(resolved_templates) > 20:
                                    st.caption(f"... y {len(resolved_templates) - 20} patrones más.")
                            else:
                                st.info("Todos los patrones de error de la Ejecución A siguen ocurriendo en la Ejecución B.")
                else:
                    # Vista Individual con pestañas para inspección o búsqueda global
                    view_tab1, view_tab2 = st.tabs(["📂 Inspeccionar Ejecución", "🔎 Buscador Global en Historial"])
                    ...
                    with view_tab1:
                        ...
                        selected_row = runs_df[runs_df["display_name"] == selected_display].iloc[0]
                        ...
                        h_col4.metric("Errores/Críticos Guardados", summary_db["errors"])
                        ...
                        class HistoricalResult:
                            def __init__(self, run_id, log_source, silhouette_score=0.0, davies_bouldin_index=0.0):
                                self.run_id = run_id
                                ...
                                self.stages = [
                                    {"stage": "Perfil del archivo", "status": "Completado", "detail": "Restaurado del historial"},
                                    {"stage": "Ingestion + limpieza", "status": "Completado", "detail": f"{int(selected_row['total_events'])} alertas guardadas"},
                                    {"stage": "Persistencia", "status": "Completado", "detail": "SQLite Historial"}
                                ]
                        
                        hist_res = HistoricalResult(sel_run_id, sel_source)
                        
                        h_html = generate_html_report(hist_incidents, hist_res)
                        h_json = generate_json_report(hist_incidents, hist_res)
                        h_csv = generate_csv_report(hist_incidents)
                        ...
                        with h_dl4:
                            if h_xlsx_available and len(h_xlsx) > 200:
                                st.download_button(
                        ...
                        st.dataframe(
                            ...
                            hide_index=True,
                            column_config={
                                "line_id": st.column_config.NumberColumn("Linea", width="small"),
                                "level": st.column_config.TextColumn("Severidad", width="small"),
                                "raw_log": st.column_config.TextColumn("Log original", width="large"),
                                "event_template": st.column_config.TextColumn("Plantilla Drain", width="medium"),
                                "anomaly_score": st.column_config.NumberColumn("Score ML", format="%.4f"),
                                "root_cause": st.column_config.TextColumn("Causa probable", width="medium"),
                                "recommendation": st.column_config.TextColumn("Recomendacion", width="large"),
                            }
                        )
                        ...
                        del_confirm = st.checkbox("Confirmar que deseo eliminar esta ejecución de la base de datos de manera permanente", key=f"del_conf_{sel_run_id}")
                        if st.button("🗑️ Eliminar ejecución de la Base de Datos", type="primary", disabled=not del_confirm, key=f"del_btn_{sel_run_id}"):
                            delete_run(sel_run_id)
                            st.success(f"Ejecución '{sel_run_id}' eliminada correctamente.")
                            st.rerun()
                            
                    with view_tab2:
                        st.markdown("##### 🔎 Buscar incidentes en todo el historial SQLite")
                        q_global = st.text_input("Término de búsqueda (Ej: Connection, Exception, Error)", value="", key="q_global_hist")
                        if q_global:
                            search_results = search_all_incidents(q_global)
                            if search_results.empty:
                                st.info("No se encontraron incidentes que coincidan con la búsqueda.")
                            else:
                                st.markdown(f"Se encontraron **{len(search_results)}** incidentes históricos:")
                                st.dataframe(
                                    search_results,
                                    width="stretch",
                                    hide_index=True,
                                    column_config={
                                        "log_source": st.column_config.TextColumn("Archivo origen", width="medium"),
                            ...
                            st.caption("Escribe un término arriba para realizar una consulta global en la base de datos local.")
