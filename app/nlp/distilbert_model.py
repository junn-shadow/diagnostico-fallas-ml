...
class SemanticEmbedder:
    ...
    def __init__(self, model_name: str = DISTILBERT_MODEL_NAME, force_backend: str = None):
        ...
        if not SemanticEmbedder._loaded:
            try:
                ...
                SemanticEmbedder._shared_tokenizer = AutoTokenizer.from_pretrained(model_name, local_files_only=True)
                SemanticEmbedder._shared_model = AutoModel.from_pretrained(model_name, local_files_only=True)
    ...
    def encode(self, texts: list[str], batch_size: int = 32):
        ...
        if self.backend == "distilbert":
            ...
            for start in range(0, len(texts), batch_size):
                ...
                pooled = (output.last_hidden_state * attention).sum(1) / attention.sum(1).clamp(min=1)
