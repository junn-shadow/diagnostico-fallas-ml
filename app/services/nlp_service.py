...
def enrich_with_semantics(logs, backend: str = "auto"):
    ...
    logger.info("NLP backend resuelto: %s (selección usuario: %s, token HF: %s)",
                force_backend, backend, "sí" if HF_API_TOKEN else "no")
    ...
    if "event_template" in df.columns:
        ...
        logger.info("Procesando agrupamiento semántico sobre %d plantillas únicas.", len(templates))
        
        embeddings, backend_val = create_embeddings(templates, force_backend=force_backend)
        labels = semantic_clusters(embeddings).values
        template_to_cluster = dict(zip(templates, labels))
        
        df["semantic_cluster"] = df["event_template"].map(template_to_cluster).fillna(-1).astype(int)
    ...
    logger.info("Procesando agrupamiento semántico sobre %d mensajes únicos.", len(unique_texts))
    
    embeddings, backend_val = create_embeddings(unique_texts, force_backend=force_backend)
    labels = semantic_clusters(embeddings).values
